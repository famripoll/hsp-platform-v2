...
export default async function CoachStudentProfilePage({
  ...
  if (
    ...
    (coachProfile.status !== "active" && coachProfile.status !== "pending")
  ) {
    redirect("/dashboard/coach");
  }
    ...
    photos = await Promise.all(
      rows
        ...
        .map(async (r) => {
          ...
          return { id: r.id, file_path: r.file_path, file_name: r.file_name, signedUrl: data?.signedUrl ?? "" };
        })
    );
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          ...
          <div className="md:col-span-1 w-full">
            <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6 w-full">
              ...
              {/* Bats/Throws/HT-WT/Hometown Grid */}
              ...
              {/* Social Links */}
              {SOCIAL_PLATFORMS.some((p) => student[p.key]) && (
                <div className="border-t border-gray-100 pt-4 mt-4">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    {SOCIAL_PLATFORMS.filter((p) => student[p.key]).map((platform) => (
                      <a
                        key={platform.name}
                        href={formatUrl(student[platform.key] as string)}
                        target="_blank"
