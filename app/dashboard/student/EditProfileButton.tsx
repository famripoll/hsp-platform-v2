...
export default function EditProfileButton({ isParent }: { isParent: boolean }) {
  if (isParent) {
    return (
      <button
        onClick={() => alert("You are in view-only mode. Only the student can edit this profile.")}
        className="absolute top-3 right-3 p-1 rounded-md text-[#64748b] hover:text-[#d93025] hover:bg-red-50 transition-colors"
